...
//     parameter P_PRE_CHARGE = 4;  // 80ns TODO: active low signal
//     parameter P_BUFFER     = 4;  // 80ns
//     parameter P_DETECT     = 4;  // 80ns    //this comes high mid {ON, OFF}_Detect
//     parameter P_ON_DETECT  = 8;  // 160ns
//     parameter P_OFF_DETECT = 8;  // 160ns
//     parameter P_RST        = 25; // 500ns TODO: get proper value from Rui

//     // add detect pulse for the col decoder
//     // or gate reset for cols
...
module roic_sm2 (
    ...
    // Phase-Gated Analog Pulses
    output logic        pre_charge_global, // [UPDATED] Active LOW
    output logic        on_detect,
    output logic        off_detect,
    output logic        detect_pulse,      // [ADDED] Mid-read column trigger
    output logic        pixel_rst,
    ...
    always_ff @(posedge sys_clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            phase_ctr         <= '0;
            wait_ctr          <= '0;
            pre_charge_global <= 1'b1; // [FIXED] Inactive state is HIGH
            on_detect         <= 1'b0;
            off_detect        <= 1'b0;
            detect_pulse      <= 1'b0; // [ADDED]
            pixel_rst         <= 1'b0;
            sm_next_row       <= 1'b0;
            fifo_wr_en        <= 1'b0;
            event_flag        <= 2'b00;
            ...
            if (sm_enable) begin
                ...
                // Safe defaults for analog pins
                pre_charge_global <= 1'b1; // [FIXED] Inactive is HIGH
                on_detect         <= 1'b0;
                off_detect        <= 1'b0;
                detect_pulse      <= 1'b0; // [ADDED] Drops automatically
                pixel_rst         <= 1'b0;
                ...
                case (state)
                    ...
                    ST_PRE_1: begin
                        pre_charge_global <= 1'b0; // [FIXED] Active is LOW
                        if (phase_ctr >= P_PRE_CHARGE - 1) begin
                            phase_ctr <= '0;
                            state     <= ST_BUF_1;
                        end else phase_ctr <= phase_ctr + 5'd1;
                    ...
                    ST_ON_DET: begin
                        ...
                        // [ADDED] Go high during the second half of the pulse
                        if (phase_ctr >= (P_ON_DETECT - P_DETECT)) begin
                            detect_pulse <= 1'b1; 
                        end
                    ...
                    ST_OFF_DET: begin
                        ...
                        // [ADDED] Go high during the second half of the pulse
                        if (phase_ctr >= (P_OFF_DETECT - P_DETECT)) begin
                            detect_pulse <= 1'b1; 
                        end
