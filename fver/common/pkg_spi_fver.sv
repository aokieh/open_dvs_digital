...
package pkg_spi_fver;
    ...
    class class_spi_ctrl;
        ...
        task automatic trans (
            ...
            // logic [15:0] tx_bits;
            logic [ 9:0] rx_3 = '0; // Widened to 10 bits for new FIFO stream
            logic [ 9:0] rx_2 = '0;
            logic [ 9:0] rx_1 = '0;
            logic [ 9:0] rx_0 = '0;
            logic [31:0] rx_bits = '0;
            ...
            // int total_num_bits = 8 << op_0[1:0];
            int send_op_addr_size = 8;
            int send_data_size; // Removed static initialization to support dynamic sizes
            ...
            // Determine  the op_code is
            if (op_0 == READ_FIFO) begin
                // Overloading the 'data' argument to act as 'num_words'
                loop_repeat = int'(data) + 1; // +1 to account for the dummy phase
                send_data_size = 10;          // 10-bit shifts for the continuous stream
            end else begin
                loop_repeat = 1; //regular case (8, 16, 32 data)
                send_data_size = 8;
            end
            ...
            for (int j = 0; j < loop_repeat; j++) begin
                // $display ("Loop = %d", j);
                for (int i = 0; i < send_data_size; i++) begin
                    // Guard against out-of-bounds indexing if send_data_size is 10
                    if (op_0 == READ_FIFO) begin
                        spi.COPI = 4'b0000;
                    end else begin
                        spi.COPI[3] = da_3[send_data_size-1-i];
                    ...
                    spi.SCK = 1; //rising edge send in
                    // rx_bits = {rx_bits[31:0], spi.CIPO};
                    rx_3 = {rx_3[8:0],spi.CIPO[3]};
                    rx_2 = {rx_2[8:0],spi.CIPO[2]};
                    rx_1 = {rx_1[8:0],spi.CIPO[1]};
                    rx_0 = {rx_0[8:0],spi.CIPO[0]};
                    // rx_bits = { //falling edge send out
                    //  {rx_3[6:0],spi.CIPO[3]},
                    //  {rx_2[6:0],spi.CIPO[2]},
                    //  {rx_1[6:0],spi.CIPO[1]},
            ...
            // Slice only the lower 8 bits so regular read logic (READ_WD) doesn't break
            rx_bits = {rx_3[7:0], rx_2[7:0], rx_1[7:0], rx_0[7:0]};
