...
module final_top_tb();
    ...
    task automatic run_full_frame(
        ...
        // 0. HARDWARE RESET: Flush the rogue FIFO data and reset the Row Counters
    ...
    task automatic qspi_write_word(input logic [7:0] addr, input logic [31:0] data);
        ...
        // ==========================================
        // FIX: Allow the pipeline to flush to memory
        // ==========================================
        @(negedge clk); // Cycle 1: we_reg goes high
        @(negedge clk); // Cycle 2: regfile samples we_reg
