...
//
// Description: 
//  Testing the state control of the ROIC across various speeds AND
//  proving 14-bit dynamic phase-tuning immunity at the 256us maximum integration.
//---------------------------------------------------------------------------
...
module roic_sm_tb2();
    ...
    // Programmable Timing Inputs - UPGRADED TO 14 BITS
    ...
    initial begin
        $display("==================================================");
        $display("Starting Multi-Speed Programmable Sequencer TB...");
        $display("Proving Frame-Rate Immunity to 14-Bit Phase Tuning");
        $display("==================================================");
        ...
        // -----------------------------------------------------------
        // 12,800 ticks available in the 256us grid. 
        // We push on/off samples to 10,000 ticks (200us) each!
        // This validates the 14-bit datapath width and arithmetic limits.
        ...
        $display("\n==================================================");
        $display("ALL SPEEDS AND 14-BIT DYNAMIC TUNINGS PASSED EXACT TIMING!");
        $display("==================================================");
        $finish;
    end

    // -----------------------------------------------------------------
    // Helper Task: Set Phase Tunings (UPGRADED TO 14 BITS)
    // -----------------------------------------------------------------
    ...
    task automatic test_speed(input string name, input int target_us, input logic [7:0] p_bits);
        ...
        fork
            // Process A: Track the absolute frame boundaries
            begin
                @(posedge on_detect);  verify_timing("ON_DETECT_START",  target_ns * 1);
                @(posedge off_detect); verify_timing("OFF_DETECT_START", target_ns * 2);
                @(posedge pixel_rst);  verify_timing("PIXEL_RST_START",  target_ns * 3);
            end

            // Process B: Assert the individual pulse widths concurrently
            verify_pulse_widths();
        join
    ...
    // -----------------------------------------------------------------
    // Helper Task: Intra-Row Pulse Width Assertions
    // -----------------------------------------------------------------
    task automatic verify_pulse_widths();
        fork
            begin : chk_pre
                time t_start, t_end; int ticks;
                @(negedge pre_charge_global); t_start = $time;
                @(posedge pre_charge_global); t_end   = $time;
            ...
            begin : chk_on
                time t_start, t_end; int ticks;
                @(posedge on_detect); t_start = $time;
                @(negedge on_detect); t_end   = $time;
                ticks = (t_end - t_start) / SYS_CLK_PERIOD_NS;
            ...
            begin : chk_off
                time t_start, t_end; int ticks;
                @(posedge off_detect); t_start = $time;
                @(negedge off_detect); t_end   = $time;
                ticks = (t_end - t_start) / SYS_CLK_PERIOD_NS;
            ...
            begin : chk_rst
                time t_start, t_end; int ticks;
                @(posedge pixel_rst); t_start = $time;
                @(negedge pixel_rst); t_end   = $time;
                ticks = (t_end - t_start) / SYS_CLK_PERIOD_NS;
