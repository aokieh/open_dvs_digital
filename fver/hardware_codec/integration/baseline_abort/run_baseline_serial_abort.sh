...
readonly FROZEN_FINAL_RELATIVE='fver/hardware_codec/integration/baseline_abort/frozen/final_top3.a26b0a8b.sv'
readonly FROZEN_RUNNER_RELATIVE='fver/hardware_codec/integration/baseline_abort/frozen/run_baseline_serial_abort.ecf2d2cd.sh'
readonly FROZEN_RUNNER_SHA256='ecf2d2cd9a56e24cb35ce8e941bef206486851774c017cc9da21be96bd971be8'
...
resolve_paths() {
    ...
    FROZEN_RUNNER="$ROOT/$FROZEN_RUNNER_RELATIVE"
    local relative
...
verify_contract_files() {
    ...
    require_hash "$FROZEN_RUNNER" "$FROZEN_RUNNER_SHA256"
...
verify_corrected_source_identity() {
    local index actual manifest_relative
    ...
    for ((index = 0; index < ${#SOURCE_RELATIVE[@]}; index += 1)); do
        ...
        manifest_relative=${SOURCE_RELATIVE[$index]}
        if [[ $manifest_relative == "$FROZEN_FINAL_RELATIVE" ]]; then
            manifest_relative='source/design/final_macros/final_top3.sv'
        fi
        printf '%s\t%s\n' "$manifest_relative" "$actual" >>"$manifest"
    done
...
make_raw_async_control() {
    ...
    while IFS= read -r line || [[ -n $line ]]; do
        ...
        printf '%s\n' "$line" >>"$final_output"
    done <"$ROOT/$FROZEN_FINAL_RELATIVE"
...
compile_and_reject_raw_async_control() {
    ...
    for ((index = 0; index < ${#SOURCE_RELATIVE[@]}; index += 1)); do
        ...
        case $relative in
            "$FROZEN_FINAL_RELATIVE") source_path=$final_control ;;
