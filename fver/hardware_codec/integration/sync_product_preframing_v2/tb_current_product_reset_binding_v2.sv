...
module tb_current_product_reset_binding_v2;
    logic sys_clk;
    logic rst_n;
    logic fifo_rst_n;
    logic fsm_rst_n;
    ...
    task automatic check_effective_resets(
        ...
        begin
            ...
            if ((dut.fsm_row_rst_n_top !== expected_row_top) ||
                ...
                $display("@@SYNC_PRODUCT_PREFRAMING_V2_RESET_FAIL@@ check=%s expected=%b%b%b%b actual=%b%b%b%b",
                         check_name,
                         expected_row_top,
                         expected_row_bottom,
                         expected_column_top,
                         ...
                         dut.col_rst_n_top,
                         dut.col_rst_n_bot);
                $fatal(1, "Current-product reset binding failed");
            end
    ...
    initial begin
        ...
        if (checks_completed != 11) begin
            $display("@@SYNC_PRODUCT_PREFRAMING_V2_RESET_FAIL@@ check=check-count expected=11 actual=%0d",
                     checks_completed);
            $fatal(1, "Current-product reset check count was wrong");
        end

        $display("@@SYNC_PRODUCT_PREFRAMING_V2_RESET_PASS@@ checks=11 reset_equations=4");
        $finish;
    end
endmodule
