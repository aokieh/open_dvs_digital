...
readonly TEST_DIR="$ROOT/fver/hardware_codec/integration/sync_product_preframing"
readonly CHECKER="$TEST_DIR/check_sync_product_preframing.py"
readonly FIXTURE_LIST="$TEST_DIR/fixture_sources.f"
readonly PRODUCT_LIST="$TEST_DIR/product_sources.f"
readonly TESTBENCH="$TEST_DIR/tb_sync_product_preframing.sv"
readonly TEST_SEAL="$TEST_DIR/test-source.sha256"
readonly SPEC="/home/rpgraca/opencode-sessions/dvs-encoder/.ocs/artifacts/files/aed-codec-campaign-v0/work/hardware-codec-v1/digital-repository-integration/sync-product-preframing-integration-unit-v1.md"
readonly CORE="$ROOT/source/design/hardware_codec/sync/opendvs_sync_product_encoder_core.sv"
...
readonly PREFLIGHT_MARKER='@@SYNC_PRODUCT_PREFRAMING_PREFLIGHT_PASS@@ fixture_elaborated=1 behavior_run=0 xcelium=21.09-s006'
readonly SELF_TEST_MARKER='@@SYNC_PRODUCT_PREFRAMING_SELF_TEST_PASS@@ fixture_green=1 semantic_plants=10 structural_controls=5'
readonly GREEN_MARKER='@@SYNC_PRODUCT_PREFRAMING_GREEN_PASS@@ integration=1 plants=10 manifests=3 ownership_regressions=2 abort_regressions=1 core_regression=1 reset_regression=1'
readonly BLOCKED_TOKEN='@@SYNC_PRODUCT_PREFRAMING_XCELIUM_BLOCKED@@'
...
fail() {
    printf '@@SYNC_PRODUCT_PREFRAMING_RUNNER_FAIL@@ message=%s\n' "$*" >&2
    exit 2
}
...
prepare_scratch() {
    ...
    SCRATCH=$(mktemp -d "$SCRATCH_PARENT/sync-product-preframing.XXXXXXXXXX") ||
        fail "could_not_create_scratch"
    case $SCRATCH in
        "$SCRATCH_PARENT"/sync-product-preframing.*) ;;
...
cleanup() {
    ...
    if [[ -n ${SCRATCH:-} && -d ${SCRATCH:-} ]]; then
        if (( rc == 0 )); then
            ...
            printf '@@SYNC_PRODUCT_PREFRAMING_SCRATCH_PRESERVED@@ path=%s\n' "$SCRATCH" >&2
        fi
    fi
    exit "$rc"
}
...
verify_fixed_paths() {
    [[ $TEST_DIR == "$ROOT/fver/hardware_codec/integration/sync_product_preframing" ]] ||
        fail "runner_is_not_at_the_frozen_path"
    local path
    for path in "$CHECKER" "$FIXTURE_LIST" "$PRODUCT_LIST" "$TESTBENCH" \
                "$TEST_SEAL" "$SPEC" "$CORE"; do
...
seal = pathlib.Path(sys.argv[2])
expected_paths = (
    "fver/hardware_codec/integration/sync_product_preframing/README.md",
    "fver/hardware_codec/integration/sync_product_preframing/check_sync_product_preframing.py",
    "fver/hardware_codec/integration/sync_product_preframing/fixture_sources.f",
    "fver/hardware_codec/integration/sync_product_preframing/fixture_sync_product_preframing.sv",
    "fver/hardware_codec/integration/sync_product_preframing/product_sources.f",
    "fver/hardware_codec/integration/sync_product_preframing/run_sync_product_preframing.sh",
    "fver/hardware_codec/integration/sync_product_preframing/tb_sync_product_preframing.sv",
)
observed = []
for line in seal.read_text(encoding="utf-8").splitlines():
    match = re.fullmatch(r"([0-9a-f]{64})  ([^\0]+)", line)
    if not match:
        raise SystemExit(f"invalid seal row: {line!r}")
    digest, relative = match.groups()
    path = root / relative
    if not path.is_file() or path.is_symlink():
        raise SystemExit(f"sealed path is absent, non-regular, or a symlink: {relative}")
    actual = hashlib.sha256(path.read_bytes()).hexdigest()
    if actual != digest:
        raise SystemExit(
            f"test-source hash mismatch: {relative} expected={digest} actual={actual}"
        )
    observed.append(relative)
if tuple(observed) != expected_paths:
    raise SystemExit(f"test-source inventory differs: {observed!r}")
print("@@SYNC_PRODUCT_PREFRAMING_TEST_SEAL_PASS@@ files=7")
PY
}
...
PY
    printf '@@SYNC_PRODUCT_PREFRAMING_XCELIUM_IDENTITY@@ version=%s sha256=%s\n' \
...
run_xcelium() {
    ...
    for ((attempt = 1; attempt <= LICENSE_ATTEMPTS; attempt += 1)); do
        ...
        if (( rc == 0 )); then
            [[ $expectation == zero ]] ||
                fail "${stage}_expected_nonzero_but_passed"
            printf '%s\n' "$stage_dir"
            return 0
        fi
...
elaborate_filelist() {
    ...
    run_xcelium "$name" zero "$ELABORATION_TIMEOUT_SECONDS" "$ROOT" \
        ...
        -top tb_sync_product_preframing -elaborate -snapshot "$snapshot" \
...
validate_plant_log() {
    ...
    printf '@@SYNC_PRODUCT_PREFRAMING_PLANT_DETECTED@@ plant=%s check=%s\n' \
        "$plant" "$check_name"
}
...
run_fixture_preflight() {
    ...
    identity=$(elaborate_filelist fixture-elaboration "$FIXTURE_LIST" sync_pf_fixture)
...
run_manifest_elaboration() {
    ...
    run_xcelium "manifest-$name" zero "$ELABORATION_TIMEOUT_SECONDS" "$workdir" \
        ...
        -xmlibdirname "$library" -l "$log" -f "$filelist" >/dev/null
    printf '@@SYNC_PRODUCT_PREFRAMING_MANIFEST_ELABORATED@@ name=%s top=%s\n' \
        "$name" "$top"
}
...
integration = str(integration_tb.relative_to(pathlib.Path.cwd()))
if lines.count(integration) != 1:
    raise SystemExit("integration testbench is not unique in product_sources.f")
...
PY
    ...
    printf '@@SYNC_PRODUCT_PREFRAMING_EXISTING_TEST_PASS@@ name=%s xcelium=1\n' "$name"
}
...
run_command_regression() {
    ...
    printf '@@SYNC_PRODUCT_PREFRAMING_EXISTING_COMMAND_PASS@@ name=%s\n' "$name"
}
...
run_green_regressions() {
    ...
    run_command_regression baseline-reset \
        '@@NO_ENCODER_BASELINE_RESET_GATE_PASS@@ strict_parse=1 structure=1 behavior=1 controls=4 repository_unchanged=1' \
        /usr/bin/bash "$ROOT/fver/hardware_codec/integration/baseline_reset/run_baseline_reset_binding.sh"
...
main() {
    ...
    case $1 in
        --expect-red)
            run_structural_checker expect-red
            verify_test_seal
            ;;
        --preflight)
            run_structural_checker preflight
            select_xcelium
        ...
        --self-test)
            run_structural_checker preflight
            run_structural_checker self-test
            select_xcelium
            ...
            ;;
        --expect-green)
            run_structural_checker expect-green
            select_xcelium
            local product_identity product_snapshot product_library ignored
            product_identity=$(elaborate_filelist product-elaboration "$PRODUCT_LIST" sync_pf_product)
